...
use <?= ltrim($generator->modelClass, '\\') ?>Base;
...
$check_datetime = array_intersect(Helper::autoSetFields(), $columnsNames);
?>
                ...
                'rules' => [
                    [
                        ...
                        'matchCallback' => function ($rule, $action) {
                            return AccessRule::matchRights(StringHelper::basename(<?= $modelClass ?>::className()), $action->id);
                        },
                    ],
                    [
                        'actions' => ['update'],
...
<?php if((count($check_datetime) != 0) || count($file_fields) != 0){ ?>
                $time_string = time();
<?php } ?>
...
<?php if(in_array('created_date', $check_datetime)) { ?>
                $model->created_date = date('Y-m-d H:i:s', $time_string);
<?php } elseif(in_array('created_at', $check_datetime)) { ?>
...
<?php endif; ?>
<?php if((count($check_datetime) != 0)){ ?>
<?php if(in_array('modified_date', $check_datetime)) { ?>
                $model->modified_date = date('Y-m-d H:i:s', $time_string);
<?php } elseif(in_array('updated_at', $check_datetime)) { ?>
...
<?php endif; ?>
    ...
    public function actionBulkAction($action = '', $field = '', $value = '')
    {
        ...
        foreach ($pks as $pk) {
            //$model = $this->findModel($pk);
            $model = <?= $modelClass ?>Base::findOne($pk);
            if (isset($model)) {
