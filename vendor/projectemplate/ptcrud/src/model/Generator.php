...
namespace projectemplate\ptcrud\model;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            if (count($all_file_fields) != 0) {
                $upload_folder = Inflector::camel2id(StringHelper::basename($modelClassName));
                $upload_folder_path = Yii::getAlias("@" . UPLOAD_DIR) . DS . $upload_folder;
                if (!is_dir($upload_folder_path)) {
                    //create folder
                    if (mkdir($upload_folder_path)) {
                        //$destination_file = $upload_folder_path . DS . ".gitignore";
                        //create .gitignore
                        /* $content = "*\n!.gitignore";
                         $fp = fopen($destination_file,"wb");
                         fwrite($fp,$content);
                         fclose($fp);*/
                        //copy .gitignore from runtime
                        $source_file = Yii::getAlias('@runtime') . DS . ".gitignore";
                        $destination_file = $upload_folder_path . DS . ".gitignore";
